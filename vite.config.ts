    ...
    types: [
      {
        ...
        realPositionCount: s.realPositionCount ?? s.realPositionGroupCount ?? lanes.reduce((n, l) => n + Number(l.realPositionGroupCount ?? l.realPositionCount ?? 0), 0),
        realPositionGroupCount: s.realPositionGroupCount ?? s.realPositionCount,
        realOrderCount: s.realOrderCount ?? lanes.reduce((n, l) => n + Number(l.realOrderCount || 0), 0),
      ...
      ...lanes.map((l) => ({
        ...
        realPositionCount: l.realPositionCount ?? l.realPositionGroupCount,
        realPositionGroupCount: l.realPositionGroupCount ?? l.realPositionCount,
        realOrderCount: l.realOrderCount,
        exchangeOpenCount: l.exchangeOpenCount,
        livePositionCount: l.livePositionCount,
        liveOrderCount: l.liveOrderCount,
        alive: l.alive,
...
function statsFallback(conn: string): Record<string, unknown> {
  ...
  const mkLane = (type: string, id: string, label: string, unit: string, exchange: string) => {
    ...
    return {
      ...
      realPositionCount: cur.realPositionCount ?? cur.realPositionGroupCount ?? 0,
      realPositionGroupCount: cur.realPositionGroupCount ?? cur.realPositionCount ?? 0,
      realOrderCount: cur.realOrderCount ?? 0,
      livePositionCount: cur.livePositionCount ?? cur.exchangeOpenCount ?? -1,
      liveOrderCount: cur.liveOrderCount ?? -1,
      wins: cur.wins ?? 0,
      losses: cur.losses ?? 0,
