...
def overlay_test() -> None:
    for name in ("overlay-bingx-x01.json", "overlay-bingx-x02.json"):
        ...
        rec(f"{name}-block-overall", ov.get("blockOverall", True) is True)
    ...
    rec("x01-block-multi", int(x01.get("blockMaxStack") or 0) == 6, str(x01.get("blockMaxStack")))
    ...
    rec("x02-listed-universe-50", x02.get("symbolsAll") is False and int(x02.get("symbolCap") or 0) == 50 and len(x02.get("symbols") or []) == 50)
    ...
    rec("x02-unlim-stack", int(x02.get("blockMaxStack") or 0) == 6 and int(x02.get("dcaMaxSteps") or 0) == 4)
