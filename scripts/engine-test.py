...
def overlay_test() -> None:
    for name in ("overlay-bingx-x01.json", "overlay-bingx-x02.json"):
        ...
        rec(f"{name}-direct-risk-range", ov.get("slMaxPct") == 2.5 and ov.get("tpMinPct") == 0.3 and ov.get("tpMaxPct") == 5.0)
        rec(f"{name}-control-range", ov.get("controlSlPct") == 2.5 and ov.get("controlTpPct") == 5.0)
        rec(f"{name}-live-gates", all(ov.get(k) == 1.25 for k in ("minPf", "baseMinPf", "mainMinPf", "realMinPf", "setMinPf", "exitMinPf")))
        rec(f"{name}-adjusted-only", ov.get("normalExecutionEnabled") is False and ov.get("blockVolumeRatio") == 2)
