...
class DynamicCostPolicyTests(unittest.TestCase):
    def test_additional_engines_share_floor_and_reject_exact_105(self):
        ...
        self.assertFalse(any(row['qualified'] for row in coord.axis_variants('parent', rows, [])))
        rows = [dict(row, pnl=.3, pnl_pct=.003) for row in rows]
        dca.closes = rows; lane.rows = rows; exits._score(lane)
        self.assertTrue(dca.score()['active']); self.assertTrue(lane.active)
        self.assertTrue(any(row['qualified'] for row in coord.axis_variants('parent', rows, [])))
