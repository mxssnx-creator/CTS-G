...
def section(source,evidence,out,table,fmt,esc,svg):
    ...
    parts=['<section id="policy"><h2>Aktuelle Regeln · PF 1,02 · DD-Zeit höchstens 16 Stunden</h2>',
        table(['Regel','Sollwert / Modell','Status'],[
            ['Mindest-PF','1,02 einschließlich Grenzwert','Neue lokale Defaults für Base/Main/Real, Sets, DCA und Exits; konfigurierte Overall-PF-Grenze gilt auch für Block Active.'],
            ...
            ['Fallback PositionCost','0,10 % Roundtrip','Historischer Lauf: keine zeitbezogenen Exchange-Gebührenproben vorhanden; das Replay zieht 0,05 % je ausgeführtem Entry/Add/Exit-Notional (0,10 % Roundtrip bei Entry + Exit) ab.'],
            ...
            ['Live-Ausführung','Block → Active; Normal (General) disabled; bis zu 80 qualifizierte Sets','Keine erzwungene Auffüllung. Research-Ergebnisse werden nicht automatisch live aktiviert.']]),
        '<p><strong>PF-Skalen:</strong> Die Ranglisten zeigen klassischen Netto-PF (Summe positiver Nettoabschlüsse / Betrag negativer Nettoabschlüsse). Die Runtime verwendet CTS-Kosten-PF: 1 + durchschnittliches Netto-Ergebnis in Kosten-Einheiten × 0,10. Der Sollwert 1,02 wird in der jeweils ausdrücklich genannten Skala geprüft; die Skalen sind nicht austauschbar.</p>',
        '<p><strong>Research-Qualifikation:</strong> Mindestens acht Abschlüsse und klassischer Netto-PF ≥ 1,02 im Training sowie im Kontrollabschnitt; maximale Drawdown-Zeit über das ganze Fenster ≤ 16 Stunden. Die Drawdown-Zeit misst Zeit unter dem bisherigen Equity-Hoch. Ein Positions-Zeitstop garantiert keine maximale Portfolio-Erholungszeit.</p>',
    ...
    ax.barh(kinds,numbers,color=['#0d9488' if v>=1.02 else '#b45309' for v in numbers])
    ax.axvline(1.02,color='#dc2626',ls='--',label='Mindest-PF 1,02')
