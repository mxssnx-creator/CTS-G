...
# Research grid deliberately includes larger per-level Block ratios.  The
# runtime cap remains two times the parent quantity; saturated cells stay in
# the report so identical outcomes can be explained rather than hidden.
BLOCK_RATIOS=(.25,.5,.75,1.,1.5)
...
WINDOW_BARS={'20h':1200,'5d':7200,'14d':20160,'20d':28800}

def select_top25(source, count=25, window='20d', qualified_only=False):
    g=grids();short=[];examined=eligible=0
    if window not in WINDOW_BARS:
        raise ValueError(f'Unsupported selection window: {window}')
    for path in sorted(pathlib.Path(source).glob(f'{window}_*.json.gz')):
        ...
        for values in packed['rows']:
            ...
            if qualified_only and not bool(values[ix['qualified']]):continue
            eligible+=1
            ...
            row=dict(identity=key,candidateId=hashlib.sha256(key.encode()).hexdigest()[:12],
                ...
                window=window,windowBars=WINDOW_BARS[window],
...
def variants(parent,admission='strict'):
    ...
    for axis,count in [('',0)]+AXES:
        for blocks,ratio in [(0,0.)]+[(n,r) for n in range(1,7) for r in BLOCK_RATIOS]:
            cfg=dict(parent)
    ...
    for i in range(60,len(bars)):
        ...
        if changed:
            ...
            last_block=np.array([bool(allow and n<=stack and (recent['count']<8 or recent['ratio']>=coord.min_pf)
                ...
                for n in range(1,7) for r in BLOCK_RATIOS])
...
def evaluate_one(args):
    ...
    window_bars=int(candidate.get('windowBars') or len(blob['bars']))
    bars=blob['bars'][-(window_bars+60):];signals=blob['signals'][candidate['kind']][-(window_bars+60):]
    ...
    block_index=np.array([(c['levels']-1)*len(BLOCK_RATIOS)+BLOCK_RATIOS.index(c['volumeRatio'])
                          if c['levels'] else -1 for c in all_cfg])
...
def main():
    ...
    p.add_argument('--output',required=True);p.add_argument('--workers',type=int,default=2);p.add_argument('--select-only',action='store_true')
    p.add_argument('--count',type=int,default=25);p.add_argument('--window',choices=sorted(WINDOW_BARS),default='20d')
    p.add_argument('--qualified-only',action='store_true');a=p.parse_args()
    out=pathlib.Path(a.output);out.mkdir(parents=True,exist_ok=True)
    chosen=select_top25(a.source,a.count,a.window,a.qualified_only);(out/'top25-selection.json').write_text(json.dumps(chosen,indent=2))
    print(f'Selected {a.count} distinct {a.window} parent configurations',flush=True)
