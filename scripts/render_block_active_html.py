"""Readable Block Active supplement, including every tested parent/ratio lane."""
import json
import pathlib
...
def section(evidence, out, table, fmt, esc, svg):
    ...
    ax.set_xlabel(f"Trainingsrang der {data['parents']} Kandidaten");ax.set_ylabel('Klassischer Netto-PF')
    ...
    parts=['<section id="block-active"><h2>Block → Active · angepasste Exchange-Ausführung</h2>',
           '<div class="notice">Die '+fmt(data['parents'],0)+' Kandidaten und '+fmt(data['variants'],0)+' Ratio-Varianten sind Research-Kandidaten: '+fmt(data['qualified'],0)+' Varianten bestehen die Ergebnisprüfung. '+fmt(sum(r['n'] for r in rows),0)+' modellierte Abschlüsse. Ein Ergebnis von null bei gesperrten Einstiegen ist kein positiver Handelsnachweis.</div>',
           table(['Setting','Neuer Default','Wirkung'],[
               ...
               ['Max active Sets','80','Auswahl begrenzt qualifizierte aktive Kandidaten; keine Auffüllung mit Verlust-Sets. Explizit 0 bleibt unbegrenzt.'],
           ...
           '<h3>Alle '+fmt(data['parents'],0)+' Kandidaten und '+fmt(data['variants'],0)+' angepassten Varianten</h3><p>Je Kandidat Counts 1–6 × Ratios 0,25 / 0,50 / 0,75 / 1,00 / 1,50; der Runtime-Deckel bleibt 2× der Referenzmenge. Die normale Menge bleibt null; jede Variante ist eine unabhängige Alternative. Auswahl im '+esc(data['selection'].get('window','20h'))+'-Training. Historische Shadow-Evidenz ersetzt im Experiment die Account-Koordination; Exchange-Mindestmengen, Funding, Latenz und ein gemeinsames Portfolio werden nicht simuliert. Die Minutenkerze prüft die Fortsetzung gröber als die Laufzeitregel.</p>']
    for parent in parents:
        c=parent['candidate'];b=parent['baseline']
        parts.append('<details class="active-parent"><summary>#'+str(c['rank'])+' · '+esc(c['symbol']+' · '+c['kind']+' · '+c['direction'])+' · '+fmt(len(parent['results']),0)+' Varianten</summary>')
