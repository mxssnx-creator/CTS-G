 ...
 *
 * Secrets are supplied only through the server environment. A missing preview
 * secret must not silently use a shared credential embedded in the repository.
 * Production uses its own GROK_AUTH_* configuration. Rotate any previously
 * published broker credential with the broker owner; deleting a source literal
 * cannot revoke a value already present in Git history.
 */
